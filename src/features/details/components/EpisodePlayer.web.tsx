...
import { resolveAnime1VStream, buildVideoProxyUrl, isValidMediaUrl, isDirectMediaUrl, Anime1VStreamLink } from '../../../../services/anime1v';
...
const isPlayerZillaUrl = (u: string): boolean =>
  /streamwish|zilla|strw|awish|playerzilla/i.test(u);

const isMp4UploadUrl = (u: string): boolean => /mp4upload/i.test(u);

const isByseUrl = (u: string): boolean => /byse|byselapuix/i.test(u);

const isVoeUrl = (u: string): boolean => /voe\.|voe\.sx|\bvoe\b/i.test(u);

export const EpisodePlayer: React.FC<EpisodePlayerProps> = ({ url, servers }) => {
  ...
  const retryResolveCountRef = useRef(0);
  const [resolveNonce, setResolveNonce] = useState(0);

  const isMp4Upload = isMp4UploadUrl(activeUrl ?? '');
  ...
  const getFallbackMp4Url = useCallback((): string | null => {
    if (!isPlayerZillaUrl(activeUrl ?? '')) return null;
    if (!servers || servers.length === 0) return null;
    const found =
      servers.find((s) => s.server.toLowerCase().includes('mp4upload')) ??
      servers.find((s) => isMp4UploadUrl(s.url));
    const fallbackUrl = found?.url ?? null;
    if (fallbackUrl && fallbackUrl !== activeUrl) return fallbackUrl;
    return null;
  }, [servers, activeUrl]);

  const handleServerFallback = useCallback(() => {
    const mp4Url = getFallbackMp4Url();
    if (mp4Url) {
      console.warn('[EpisodePlayer] PlayerZilla no pudo cargar, cambiando a MP4Upload');
      stopVideo();
  ...
  }, [getFallbackMp4Url, stopVideo]);

  const retryResolve = useCallback(() => {
    console.warn('[EpisodePlayer] Re-resolviendo con token fresco...');
  ...
  useEffect(() => {
    ...
    retryResolveCountRef.current = 0;
    setActiveUrl(url);
    setPlayerState('loading');
    setStreamUrl(null);
    setMediaType(null);
  ...
  useEffect(() => {
    ...
    if (isHls || isMp4) {
      setResolving(false);
      // Directo también por el proxy (Referer/Range/206 del lado servidor)
      setStreamUrl(buildVideoProxyUrl(activeUrl));
      setMediaType(isHls ? 'hls' : 'mp4');
      setPlayerState('stream_ready');
      return;
    }

    // Byse no se puede resolver en backend; reproducir el embed directo (iframe)
    if (isByseUrl(lowerUrl)) {
      setResolving(false);
      setPlayerState('iframe');
      return;
    }
    ...
    resolveAnime1VStream(activeUrl)
      .then((res) => {
        if (cancelled) return;
        // VOE: solo aceptar resolución si trae media directa; si no, reproducir el embed sin resolver
        const voeStreamOk = !isVoeUrl(lowerUrl) || isDirectMediaUrl(res?.streamUrl ?? '');
        if (voeStreamOk && res?.success && res.streamUrl && (res.mediaType === 'hls' || res.mediaType === 'mp4') && isValidMediaUrl(res.streamUrl)) {
          hlsFatalCountRef.current = 0;
          retryResolveCountRef.current = 0;
          // Siempre por el proxy: el navegador no puede fijar Referer y sin él
          // mp4upload devuelve 403. El proxy reenvía Range y responde 206.
          setStreamUrl(buildVideoProxyUrl(res.streamUrl));
          setMediaType(res.mediaType);
          setPlayerState('stream_ready');
        } else {
          if (isVoeUrl(lowerUrl)) {
            console.warn('[EpisodePlayer] VOE sin resolver directo, reproduciendo embed sin resolver');
          }
          handleServerFallback();
        }
      })
      .catch(() => {
        if (!cancelled) handleServerFallback();
      })
      .finally(() => {
  ...
  }, [activeUrl, handleServerFallback, resolveNonce]);
    ...
    if (mediaType === 'hls') {
      const startHls = (source: string) => {
        ...
        if (!Hls.isSupported()) {
          ...
          if (vid && vid.canPlayType('application/vnd.apple.mpegurl')) {
            vid.src = source;
          }
          return;
        }
        ...
        hls.on(Hls.Events.MANIFEST_PARSED, () => {
          console.log('[EpisodePlayer] HLS MANIFEST_PARSED, lista (sin autoplay)');
        });
        hls.on(Hls.Events.ERROR, (_event, data) => {
          ...
          if (hlsFatalCountRef.current <= 2) {
            console.warn(`[EpisodePlayer] HLS fatal (intento ${hlsFatalCountRef.current}/2), recargando...`);
            startHls(source);
          } else {
            console.error('[EpisodePlayer] HLS no pudo cargarse, aplicando fallback');
            hls.destroy();
            hlsRef.current = null;
            handleServerFallback();
          }
      ...
      const onError = () => {
        ...
        if (retryResolveCountRef.current < MAX_RESOLVE_RETRIES) {
          retryResolveCountRef.current += 1;
          // El token de mp4upload se rota en cada fetch del embed; si quedó "viejo",
          // el proxy devuelve 502 (upstream 403). Re-resolver y reintentar una vez.
          console.warn(`[EpisodePlayer] MP4 error (intento ${retryResolveCountRef.current}/${MAX_RESOLVE_RETRIES}), re-resolviendo con token fresco...`);
          setTimeout(retryResolve, RESOLVE_RETRY_DELAY_MS);
        } else {
          console.warn('[EpisodePlayer] MP4 falló tras reintento, aplicando fallback');
          handleServerFallback();
        }
      };
  ...
  }, [playerState, streamUrl, mediaType, stopHls, handleServerFallback, retryResolve]);
  ...
  useEffect(() => {
    ...
    const handleDocClick = (e: MouseEvent) => {
      ...
      if (anchor && anchor.target === '_blank') {
        ...
        if (activeUrl && !href.includes(new URL(activeUrl).hostname)) {
          e.preventDefault();
          e.stopImmediatePropagation();
          showBlocked();
        }
  ...
  }, [isMp4Upload, activeUrl, showBlocked]);
